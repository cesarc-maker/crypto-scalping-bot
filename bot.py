...
TOP_MOVER_COUNT = int(os.getenv("TOP_MOVER_COUNT", 20))
MIN_QUOTE_VOL_USDT = float(os.getenv("MIN_QUOTE_VOL_USDT", 6_000_000))
...
def momentum_bullish(df_exec) -> bool:
    last = df_exec.iloc[-1]
...
def vwap_reclaim(df_exec) -> bool:
    last = df_exec.iloc[-1]
    close = float(last["close"])
    vwap = float(last["vwap"])
    return close >= vwap * (1.0 + VWAP_RECLAIM_BUFFER)

# ======================================================
# BUILD + SEND TRADE
# ======================================================

def build_trade(ex_name: str, symbol: str, entry: float, atr: float) -> Optional[Dict[str, Any]]:
    if atr <= 0:
        return None
    ...
    if stop_pct < STOP_MIN_PCT or stop_pct > STOP_MAX_PCT:
        return None
    tp = entry + (entry - stop) * RR  # strict 1:1
    now = int(time.time())
    return {
        "ex_name": ex_name,
        "symbol": symbol,
    ...
    while True:
        ...
        for k in keys:
            try:
                ...
                if px <= float(t["stop"]):
                    send_telegram(f"❌ SL HIT — {t['symbol']} ({t['ex_name']})")
                ...
                if px >= float(t["tp"]):
                    send_telegram(f"✅ TP HIT (1:1) — {t['symbol']} ({t['ex_name']})")
        ...
        for ex_name in EXCHANGES:
            ...
            for symbol in movers:
                try:
                    ...
                    if not ctx_bullish(df_ctx):
                        continue
                    if not trend_bullish(df_trend):
                        continue
                    ...
                    if pullback_ok(df_exec) and vwap_reclaim(df_exec) and momentum_bullish(df_exec):
                        if allow(ex_name, symbol, "LONG"):
                            ...
                            trade = build_trade(ex_name, symbol, entry, atr)
                            if trade:
                                send_signal(trade)
...
@app.route("/")
def home():
    return "CRT FAST BOT RUNNING (INFO ONLY) — OKX + KUCOIN"
